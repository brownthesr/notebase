...
The hardest part usually is computing the covariance and mean. Also if things are not linear then we don't necessarily have a gaussian at the end so that would make exact sampling of the perturbed $x_t$ harder since we would have to run the differential equation forward in time. What I ended up doing was using a stand form for a rotation matrix and scaling for $A$ (mainly negative scaling and constant rotation). The fundamental matrix in this case is really nice and if $g(t)=\sigma^t I$ then the rotation matrices in the $\Sigma(t)$ just cancel out and we are just left with integration over scalars. Taking the square root of a scalar is also super easy for this reason and so we lucked out.
